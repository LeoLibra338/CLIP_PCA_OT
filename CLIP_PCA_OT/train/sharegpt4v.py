...
class share4v_val_dataset(data.Dataset):
    def __init__(self, val_size=1000):
        with open(captions_json, 'r', encoding='utf8') as fp: self.json_data = json.load(fp)[:val_size]
        self.preprocess = _transform()
    def __len__(self): return len(self.json_data)
    def __getitem__(self, index):
        item = self.json_data[index]
        return self.preprocess(Image.open(os.path.join(image_dir, item['image']))), item['caption']
...
class share4v_train_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            item = self.json_data[index]
            return self.preprocess(Image.open(os.path.join(image_dir, item['image']))), item['caption'], item['caption_short']

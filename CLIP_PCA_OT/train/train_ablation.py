...
from sharegpt4v_subset import create_train_dataset
# share4v_val_dataset, share4v_train_dataset
...
class CLIP_Clean_Train():
    def __init__(self, rank, local_rank, args):
        ...
        self.model = torch.nn.parallel.DistributedDataParallel(self.model, device_ids=[local_rank], find_unused_parameters=True, broadcast_buffers=False)
    ...
    def train_epoch(self, dataloader, epoch, start_iter=0, train_losses=None):
        ...
        for i, (images, texts, _) in enumerate(tqdm(dataloader, disable=(self.rank != 0))):
            ...
            with torch.cuda.amp.autocast():    
                ...
                loss_il1, loss_il2 = self.model(images, texts, self.rank, pca_ratio=self.args.pca_ratio, queue_size=self.args.queue_size, focal_gamma=self.args.focal_gamma, gate_beta=self.args.gate_beta, gate_theta=self.args.gate_theta)
                loss = loss_il1 + self.args.ot_lambda * loss_il2
            ...
            logs = {"EP": f'{epoch}/{self.args.epochs-1}, {i}/{num_batches_per_epoch}',
                    ...
                    "gate": f"b={self.args.gate_beta}t={self.args.gate_theta}",
                    }
    ...
    def train(self, resume=False, warmup_length=200):
        trainset = create_train_dataset(max_samples=int(os.environ.get("MAX_TRAIN_SAMPLES", 50000)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--queue_size", type=int, default=4096,
        help="MoCo-style text feature queue size (0=disabled)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gate_beta", type=float, default=0.0,
        help="PCA-Gate steepness (0=disabled). Recommended: 5.0."
    )
    parser.add_argument(
        "--gate_theta", type=float, default=1.0,
        help="PCA-Gate threshold. ratio>theta → false neg; ratio<theta → hard neg."
    )
    parser.add_argument(
        "--ot_lambda", type=float, default=1.0,
        help="PCA loss weight (0=pure InfoNCE, 0.15=balanced, 1.0=equal)."
    )

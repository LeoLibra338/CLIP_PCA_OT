"""
FineLIP (ViT-L/14) 在 COCO train2017 上 fine-tune
支持 baseline 模式（纯 InfoNCE）和 finelip 模式（+ cross_net alignment）
"""
import torch
...
import sys, os, argparse, random, numpy as np

sys.path.insert(0, '/home/gyf/CLIP-wgd/FineLIP')
sys.path.insert(0, '/home/gyf/CLIP-wgd/FineLIP/model')
...
class FineLIPTrainer:
    def __init__(self, rank, local_rank, args):
        self.rank = rank
        self.local_rank = local_rank
        self.run_finelip = not args.run_baseline

        # 加载 FineLIP 模型
        self.model, _ = finelip.load_from_clip(
            args.base_model, device='cpu',
            run_finelip=self.run_finelip
        )
        ...
        if rank == 0:
            ...
            print(f'FineLIP mode: {self.run_finelip}')
            print(vars(args))

        self.args = args
        self.warmup_alpha = 0.0  # FineLIP warmup
    ...
    def train_epoch(self, dataloader, epoch, train_losses):
        ...
        for i, (images, texts, _) in enumerate(tqdm(dataloader, disable=self.rank != 0)):
            ...
            images = images.cuda()
            texts_tokenized = finelip.tokenize(texts, truncate=True).cuda()
            img_ids = torch.arange(images.size(0), device=images.device)

            # FineLIP warmup alpha: 前一半 epoch 从 0 线性到 1
            if self.run_finelip:
                total_steps = self.num_epoch * num_batches
                self.warmup_alpha = min(1.0, step / (total_steps * 0.5))

            with torch.cuda.amp.autocast():
                if self.run_finelip:
                    loss1, loss3 = self.model(images, texts_tokenized, img_ids,
                                              self.warmup_alpha, rank=self.rank)
                    loss = loss3  # cross_net alignment loss
                else:
                    # Baseline: 标准 InfoNCE
                    image_features = self.model.module.encode_image(images)
                    text_features = self.model.module.encode_text(texts_tokenized)
        ...
        for epoch in range(self.num_epoch):
            ...
            if self.rank == 0:
                torch.save(self.model.module.state_dict(),
                           os.path.join(self.save_dir, f'ep={epoch}_finelip.pt'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jobname', type=str, default='finelip_coco')
    ...
    parser.add_argument('--run_baseline', action='store_true', default=False,
                        help='Run baseline mode (standard InfoNCE, no cross_net)')
    parser.add_argument('--download-root', default=None)
    args = parser.parse_args()

    rank, local_rank = setup_distributed()
    trainer = FineLIPTrainer(rank, local_rank, args)
    trainer.train()

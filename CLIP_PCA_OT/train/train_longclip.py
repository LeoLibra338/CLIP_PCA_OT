"""
Long-CLIP (ViT-L/14, 248 token) 在 COCO train2017 上 fine-tune
使用 Long-CLIP 自带的 InfoNCE loss（非 PCA）
"""
import torch
...
sys.path.insert(0, '/home/gyf/CLIP-wgd/Long-CLIP')
...
class LongCLIPTrainer:
    def __init__(self, rank, local_rank, args):
        self.rank = rank
        self.local_rank = local_rank

        # 加载 Long-CLIP 模型（从 ViT-L/14 转换，248 token context）
        self.model, _ = longclip.load_from_clip(args.base_model, device='cpu', download_root=args.download_root)
        self.model = self.model.float()  # 确保 fp32（Long-CLIP 可能以 fp16 加载）
        self.model.train()
        # 重新设置为 fp32 parameter
    ...
    def train_epoch(self, dataloader, epoch, train_losses):
        ...
        for i, (images, texts, _) in enumerate(tqdm(dataloader, disable=self.rank != 0)):
            ...
            with torch.cuda.amp.autocast():
                ...
                bs = images.size(0)
                targets = torch.linspace(self.rank * bs, self.rank * bs + bs - 1, bs, dtype=torch.long).cuda()
    ...
    def train(self):
        ...
        loader = torch.utils.data.DataLoader(trainset, batch_size=self.batch_size, sampler=sampler, num_workers=4, pin_memory=True)
        ...
        for epoch in range(self.num_epoch):
            ...
            if self.rank == 0:
                torch.save(self.model.module.state_dict(),
                           os.path.join(self.save_dir, f'ep={epoch}_longclip.pt'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jobname', type=str, default='longclip_coco')
    ...
    trainer = LongCLIPTrainer(rank, local_rank, args)
    trainer.train()
